...
def import_functions():
    function_list = ['atlassian', 'gravatar', 'voxmedia', 'wordpress', 'aboutme', 'amocrm', 'axonaut', 'hubspot', 'insightly', 'nimble', 'nocrm', 'nutshell', 'pipedrive', 'teamleader', 'zoho', 'buymeacoffee', 'babeshows', 'badeggsonline', 'biosmods', 'biotechnologyforums', 'blackworldforum', 'blitzortung', 'bluegrassrivals', 'cambridgemt', 'chinaphonearena', 'clashfarmer', 'codeigniter', 'cpaelites', 'cpahero', 'cracked_to', 'demonforums', 'freiberg', 'koditv', 'mybb', 'nattyornot', 'ndemiccreations', 'nextpvr', 'odampublishing', 'onlinesequencer', 'raidforums', 'thecardboard', 'therianguide', 'thevapingforum', 'treasureclassifieds', 'coroflot', 'freelancer', 'seoclerks', 'diigo', 'quora', 'google', 'laposte', 'mail_ru', 'protonmail', 'yahoo', 'ello', 'flickr', 'komoot', 'rambler', 'sporcle', 'caringbridge', 'sevencups', 'blip', 'lastfm', 'smule', 'spotify', 'tunefind', 'rocketreach', 'venmo', 'pornhub', 'redtube', 'xvideos', 'anydo', 'evernote', 'eventbrite', 'nike', 'samsung', 'codecademy', 'codepen', 'devrant', 'github', 'replit', 'teamtreehouse', 'vrbo', 'amazon', 'armurerieauxerre', 'deliveroo', 'dominosfr', 'ebay', 'envato', 'garmin', 'naturabuy', 'vivino', 'bitmoji', 'crevado', 'discord', 'fanpop', 'imgur', 'instagram', 'myspace', 'odnoklassniki', 'parler', 'patreon', 'pinterest', 'plurk', 'snapchat', 'strava', 'taringa', 'tellonym', 'tumblr', 'twitter', 'vsco', 'wattpad', 'xing', 'adobe', 'archive', 'docker', 'firefox', 'issuu', 'lastpass', 'office365', 'bodybuilding', 'blablacar']
    function_ref_list=list()
    for x in function_list:
        function_ref_list.append(eval(x))
    return function_ref_list  

def get_functions(modules,args=None):
    """Transform the modules objects to functions"""
    websites = []
    print(len(modules.values()))
...
def print_result(data,args,email,start_time,websites):
    ...
    if args.noclear==False:
        print("\033[H\033[J")
    print(f"[*] {email} is used on these websites:")
